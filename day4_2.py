...
practise = """aaaaa-bbb-z-y-x-123[abxyz]
...
class Entry:
    def __init__(self,string):
        blocks = string.split('-')
        controlBlocks = blocks[-1].split('[')
        self.checkSum = controlBlocks[1][:-1]
        self.sectorID = int(controlBlocks[0])
        self.words = blocks[:-1]

    def IsReal(self):
        counts = []
        for i in range(0,26):
            counts.append( (0,chr(97+i)) )
        for word in self.words:
            for letter in word:
                index = ord(letter)-97
                counts[index] = (counts[index][0]+1,letter)
        ordered = sorted(counts, key = operator.itemgetter(0), reverse=True) #stable
        mostFive = "".join([x[1] for x in ordered[0:5]])
        return mostFive == self.checkSum 
    ...
    def Decrypt(self):
        ...
        for word in self.words:
            realWord = ""
            for letter in word:
                shifted = (ord(letter)-97 + self.sectorID) % 26
                realWord += chr(shifted+97)
...
for line in input.split('\n'):
    entry = Entry(line)
    if entry.IsReal():
        print entry.Decrypt() + " : " + str(entry.sectorID)
        sum += entry.sectorID
print sum
