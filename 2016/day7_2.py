...
example  = "aba[bab]xyz"
example2 = "xyx[xyx]xyx"

def IsSupportSSL(line):
	result = False
	isIn = False
	aba = []
	bab = []
	for i in range(2,len(line)):
		if line[i] == '[' or line[i] == ']':
			isIn = not isIn
		elif line[i-1] != '[' and line[i-1] != ']' and line[i] ==line[i-2] and line[i-1] != line[i]:
			mirror = line[i-1]+line[i]+line[i-1]
			if isIn:
				if	mirror in aba:
					return True
				else:
					bab.append(line[i-2:i+1])
			else:
				if mirror in bab:
					return True
				else:
					aba.append(line[i-2:i+1])
	return False
